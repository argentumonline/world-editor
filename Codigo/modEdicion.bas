...
Public MapCopy(1 To 100, 1 To 100) As MapBlock
Public MinCopyX As Byte
...
Public Sub CopiarSeleccion()
    ...
    If MaxSelectX = 0 Then Exit Sub
    
    For Y = MinSelectY To MaxSelectY
        For X = MinSelectX To MaxSelectX
            MapData(X, Y).Select = 0
            MapCopy(X, Y) = MapData(X, Y)
        Next X
    Next Y
...
Public Sub PegarSeleccion()
    Dim X As Byte
    Dim Y As Byte
    Dim Width As Byte
    Dim Height As Byte
    Dim ActWidth As Byte
    Dim ActHeight As Byte
    
    If MaxSelectX = 0 Or MaxCopyX = 0 Then Exit Sub
    
    Width = MaxCopyX - MinCopyX + 1
    Height = MaxCopyY - MinCopyY + 1
    
    For Y = MinCopyY To MaxCopyY
        If MinSelectY + ActHeight > 100 Then Exit Sub
        ActWidth = 0
        For X = MinCopyX To MaxCopyX
            If MinSelectX + ActWidth > 100 Then Exit Sub
            MapData(MinSelectX + ActWidth, MinSelectY + ActHeight) = MapCopy(X, Y)
            ActWidth = ActWidth + 1
        Next X
...
If MapData(X, Y).Blocked <> 1 Then
    ...
    With GrhData(BlockGrhIndex)
        Call g_Swarm.Insert(BLOCK_LAYER, -1, X, Y, .TileWidth, .TileHeight)
    End With
...
If MapData(X, Y).Blocked <> 0 Then
    ...
    With GrhData(BlockGrhIndex)
        Call g_Swarm.Remove(BLOCK_LAYER, -1, X, Y, .TileWidth, .TileHeight)
    End With
